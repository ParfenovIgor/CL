...
/* Turn on line/column tracking in the normal_lloc structure: */
%locations
...
;
Expr : Expr _KW_where Type _IDENT_ _EQ Expr { $$ = new Where($1, $3, $4, $6); result->expr_ = $$; }
  | Expr _KW_where _LBRACE ListWhereField_ _RBRACE { std::reverse($4->begin(),$4->end()) ;$$ = new MultiWhere($1, $4); result->expr_ = $$; }
  | _KW_fun _LPAREN Type _IDENT_ _RPAREN _LBRACE ListStatement _KW_ret Expr _RBRACE { std::reverse($7->begin(),$7->end()) ;$$ = new Abstraction($3, $4, $7, $9); result->expr_ = $$; }
  ...
  | Expr Expr1 { $$ = new Application($1, $2); result->expr_ = $$; }
  | _KW_uni _IDENT_ _SYMB_6 Expr { $$ = new TypeAbstraction($2, $4); result->expr_ = $$; }
  | Expr _LBRACK Type _RBRACK { $$ = new TypeApplication($1, $3); result->expr_ = $$; }
...
;
Expr6 : _KW_true { $$ = new ConstTrue(); result->expr_ = $$; }
  | _KW_false { $$ = new ConstFalse(); result->expr_ = $$; }
  | _SYMB_9 { $$ = new ConstZero(); result->expr_ = $$; }
...
;
Expr1 : _KW_if Expr1 _KW_then Expr1 _KW_else Expr1 { $$ = new If($2, $4, $6); result->expr_ = $$; }
  | _KW_succ Expr1 { $$ = new Succ($2); result->expr_ = $$; }
  | _KW_pred Expr1 { $$ = new Pred($2); result->expr_ = $$; }
  | _KW_iszero Expr1 { $$ = new IsZero($2); result->expr_ = $$; }
  | _KW_fix Expr1 { $$ = new Fix($2); result->expr_ = $$; }
  | _LT ListExpr _GT { std::reverse($2->begin(),$2->end()) ;$$ = new Tuple($2); result->expr_ = $$; }
  | Expr1 _DOT _INTEGER_ { $$ = new TupleGet($1, $3); result->expr_ = $$; }
  | _LBRACE ListRecordField_ _RBRACE { std::reverse($2->begin(),$2->end()) ;$$ = new Record($2); result->expr_ = $$; }
  | Expr1 _DOT _IDENT_ { $$ = new RecordGet($1, $3); result->expr_ = $$; }
  ...
  | _KW_get _LPAREN Expr Expr _RPAREN { $$ = new ArrayGet($3, $4); result->expr_ = $$; }
  | _KW_push _LPAREN Expr Expr _RPAREN { $$ = new ArrayPush($3, $4); result->expr_ = $$; }
...
;
Expr5 : _KW_ref Expr5 { $$ = new Reference($2); result->expr_ = $$; }
  | _STAR Expr5 { $$ = new Dereference($2); result->expr_ = $$; }
  | Expr6 { $$ = $1; result->expr_ = $$; }
;
Expr4 : Expr2 _SYMB_14 Expr2 { $$ = new AddInt($1, $3); result->expr_ = $$; }
  | Expr2 _SYMB_15 Expr2 { $$ = new SubInt($1, $3); result->expr_ = $$; }
  | Expr2 _SYMB_20 Expr2 { $$ = new AddReal($1, $3); result->expr_ = $$; }
  | Expr2 _SYMB_21 Expr2 { $$ = new SubReal($1, $3); result->expr_ = $$; }
  | Expr5 { $$ = $1; result->expr_ = $$; }
;
Expr3 : Expr2 _SYMB_16 Expr2 { $$ = new MulInt($1, $3); result->expr_ = $$; }
  | Expr2 _SYMB_17 Expr2 { $$ = new DivInt($1, $3); result->expr_ = $$; }
  | Expr2 _SYMB_22 Expr2 { $$ = new MulReal($1, $3); result->expr_ = $$; }
  | Expr2 _SYMB_23 Expr2 { $$ = new DivReal($1, $3); result->expr_ = $$; }
  | Expr4 { $$ = $1; result->expr_ = $$; }
;
Expr2 : Expr2 _SYMB_18 Expr2 { $$ = new EquInt($1, $3); result->expr_ = $$; }
  | Expr2 _SYMB_19 Expr2 { $$ = new LesInt($1, $3); result->expr_ = $$; }
  | Expr2 _SYMB_24 Expr2 { $$ = new EquReal($1, $3); result->expr_ = $$; }
  | Expr2 _SYMB_25 Expr2 { $$ = new LesReal($1, $3); result->expr_ = $$; }
...
;
WhereField_ : _IDENT_ _COLON Type _EQ Expr1 { $$ = new WhereField($1, $3, $5); result->wherefield__ = $$; }
;
...
;
Statement : _KW_import _LT _IDENT_ _GT { $$ = new Import($3); result->statement_ = $$; }
  | _KW_def Type _IDENT_ _EQ Expr { $$ = new Definition($2, $3, $5); result->statement_ = $$; }
  | Expr _LARROW Expr { $$ = new MoveAssignment($1, $3); result->statement_ = $$; }
  | Expr _COLONEQ Expr { $$ = new CopyAssignment($1, $3); result->statement_ = $$; }
  | _KW_if Expr _LBRACE ListStatement _RBRACE { std::reverse($4->begin(),$4->end()) ;$$ = new IfStatement($2, $4); result->statement_ = $$; }
  | _KW_if Expr _LBRACE ListStatement _RBRACE _KW_else _LBRACE ListStatement _RBRACE { std::reverse($4->begin(),$4->end()) ; std::reverse($8->begin(),$8->end()) ;$$ = new IfElseStatement($2, $4, $8); result->statement_ = $$; }
