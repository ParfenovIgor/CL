...
void Substitute_TypeDefinition  (Statement*&, Ident&, Expr*);
void Substitute_Assignment      (Statement*&, Ident&, Expr*);
...
void Substitute(Expr *&expr, Ident &ident, Expr *value) {
    ...
    if (dynamic_cast <Record*> (expr)) {
        Substitute_Record(expr, ident, value); return;
    }
    if (dynamic_cast <RecordGet*> (expr)) {
        Substitute_RecordGet(expr, ident, value); return;
    }
...
void Substitute(Statement *&expr, Ident &ident, Expr *value) {
    ...
    if (dynamic_cast <TypeDefinition*> (expr)) {
        Substitute_TypeDefinition(expr, ident, value); return;
    }
    if (dynamic_cast <Assignment*> (expr)) {
        Substitute_Assignment(expr, ident, value); return;
    }
...
void Substitute_TypeDefinition(Statement *&expr, Ident &ident, Expr *value) { }
void Substitute_Assignment(Statement *&expr, Ident &ident, Expr *value) {
    Assignment *assignment = dynamic_cast <Assignment*> (expr);
    Substitute(assignment->expr_1, ident, value);
    Substitute(assignment->expr_2, ident, value);
}
